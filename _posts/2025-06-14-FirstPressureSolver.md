...
Now we can run our simulation, drink a cup of coffee, maybe have full meal (or two) and then we will look at the results.

# 5. Results

After 5 minutes we can look at the first results of the breaking dam scene. For this we spawn a grid of particles and advect them according to the pressure solver. For starters we set the viscosity very low. As particle diameter we choose 0.005m and we run the simulation with 3000Hz. This means for one second of footage we need to run the neighborhood search 3000 times. In future we will explore different solvers that can hopefully run at a lower frequency. This is what makes the computation very slow. As we can see, the particles look like fluid, but non the less, we still have the look of a ball pit and not really the look of water. This issue will be fixed in a future post.
<video width="100%" controls>
  <source src="/img/posts/2025-06-13-FirstPressureSolver/DamBreak.mp4" type="video/mp4">
