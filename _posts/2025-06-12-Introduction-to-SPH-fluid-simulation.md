...
What this means, is that the closer the neighbors (the particle it self being the closest neighbor) are the higher the influence of the density will be, which is excatly what we want. Many particles clustered together should result in a high density and sparsly seeded once into a low one. This is visualized here:
